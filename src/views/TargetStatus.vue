...
// Only agents that actually record probe/interface series are worth listing.
const agentsWithTargets = computed(() => agents.value.filter((a) => (seriesByAgent.value.get(a.id)?.length ?? 0) > 0))

const currentAgentSeries = computed(() => seriesByAgent.value.get(agentId.value) ?? [])
const agentGroups = computed(() => groupTargets(currentAgentSeries.value, familyLabel, monitorNames.value))
const detailGroup = computed(() => agentGroups.value.find((g) => g.key === targetKey.value) || null)

// Union of every monitor / system target across all agents (for the by-target selector).
const allTargetGroups = computed<TargetGroup[]>(() => {
  const m = new Map<string, TargetGroup>()
  for (const [, series] of seriesByAgent.value) {
    for (const g of groupTargets(series, familyLabel, monitorNames.value)) {
...
// different monitor), by (family, target) for system series.
const probers = computed<Prober[]>(() => {
  ...
  for (const a of agents.value) {
    ...
    if (series.length) out.push({ agent: a, series })
  }
  return out
})
    ...
    <template v-else-if="ready">
      ...
      <template v-if="view === 'agent'">
        <TargetDetail
          ...
          @back="backToGrid"
        />
        <AgentTargetsGrid v-else :agent-id="agentId" :groups="agentGroups" :range-sec="rangeSec" @select="selectTarget" />
      </template>
