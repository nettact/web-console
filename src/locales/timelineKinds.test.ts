...
const TARGET_STATUS_PATHS = [
  ...['disabled', 'unassigned', 'alerting', 'breaching', 'partial_failure', 'probe_failed', 'blocked', 'agent_offline', 'pending', 'stale', 'no_data', 'healthy'].map((v) => `display.${v}`),
  ...
  ...['normal', 'breaching', 'alerting'].map((v) => `rule.${v}`),
  ...['target_disabled', 'no_applicable_agents', 'agent_offline', 'permission_blocked', 'target_blocked', 'unsupported', 'awaiting_status_report', 'alert_firing', 'rule_breaching', 'probe_failed', 'probe_stale', 'probe_no_data', 'not_applicable', 'ok'].map((v) => `reason.${v}`),
]
