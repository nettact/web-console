...
export default {
  ...
  spform: {
    newTitle: 'New status page',
    editTitle: 'Edit status page',
    sub: 'Everything on this page is visible to anyone, with no sign-in.',
    ...
    descriptionPlaceholder: 'Optional, shown under the title',
    descriptionHint: 'Displayed verbatim to every visitor.',
    ...
    homeVsDomain: 'That is the same-domain approach. To put the board on its own domain and shut the console and admin API out of it entirely, use the reverse proxy setup instead:',
    ...
    agentMetricsHint: {
      ...
      full: 'Careful: also publishes total and used memory and disk in bytes, which describe the machine itself.',
    },
    showAgentView: 'Show the node list',
    noAgentGroups: 'This site has no agent groups yet.',
    manageAgentGroups: 'Create one',
    groupMembers: '{n} agents',
    ...
    showTargetAddress: 'Also show target addresses',
    addressOffHint: 'Unnamed targets appear as placeholders like “HTTP target 1”, exposing no IP, hostname or URL.',
  ...
  incidents: {
    ...
    resolveReason: {
      ...
      subject_gone: 'The disk is no longer present',
    },
    cmp: {
      gt: 'greater than',
    ...
    detector: {
      ...
      host_cpu: 'CPU',
      host_mem: 'Memory',
      host_load: 'System load',
      host_net: 'Network rate',
      host_disk: 'Disk',
      // The two network directions. A disk's subject is its mount point and is
      // shown verbatim — it is the operator's own path, not a term to translate.
      subject_rx: 'download',
      subject_tx: 'upload',
    },
    detail: {
    ...
    snap: {
      ...
      noScenes: 'No scene has been filed for this incident. Agents collect one locally when they detect a probe fault or lose their server connection, so one collected during an outage appears after the Agent reconnects — but not every fault produces one: host and Wi-Fi faults are out of scope, path diagnostics must be enabled, and lite (router) Agents never collect scenes.',
      collectedAt: 'Collected',
      clockAhead: "This Agent's clock is about {s}s ahead of the server, so its collection time cannot be compared with the rest of this page.",
      queuedFor: 'Collected about {n} min before it reached the server — it waited in the Agent outbox until the Agent reconnected.',
      ...
      disconnectReason: {
        dns: 'server name did not resolve',
    ...
    trace: {
      ...
      noHops: 'No per-hop data was recorded.',
      starNote:
      ...
      reason: {
        ...
        local_no_route: 'No route on the Agent host',
      ...
      reasonDetail: {
        ...
        local_no_route: 'The probes never left the machine: at diagnostic time the Agent host had no usable route to this destination (no route, a next hop whose ARP went unanswered, or an egress link that lost carrier). That is the finding — the break is on the Agent’s side of the wire, not along the path, so there are no per-hop results to read. In this state the kernel answers itself with an ICMP unreachable sourced from a local address; recorded as hops, every single hop would read as the Agent’s own address.',
  ...
  processes: {
    ...
    sortRam: 'RAM',
    sortName: 'Name',
    sortRemote: 'Remote address',
    sortProcess: 'Process',
    sortState: 'Status',
    searchProcesses: 'Search processes',
    searchProcessesPlaceholder: 'Name, PID or user',
    searchConnections: 'Search connections',
    searchConnectionsPlaceholder: 'Address, process, PID or protocol',
    showAllFields: 'View all fields',
    processIdentity: 'Process / PID',
    connectionsAndDetails: 'Connections / details',
    protocolAndStatus: 'Protocol / status',
    processAndDetails: 'Process / details',
    connectionCount: '{n} connection | {n} connections',
    showDetails: 'Details',
  ...
  monitoring: {
    ...
    newHostMonitor: '+ Add system status alert',
    hostSystem: 'System metrics',
    // Which resource families a host anchor watches, shown as chips on its row.
    hostFam: {
      cpu: 'CPU',
      mem: 'Memory',
      load: 'Load',
    ...
    metric: {
      ...
      probe_http_total_ms: 'Total probe time (ms)',
      probe_http_ttfb_ms: 'Time to first byte (ms)',
      probe_http_connect_ms: 'Connect time (ms)',
      probe_http_dns_ms: 'DNS resolve time (ms)',
      probe_http_tls_ms: 'TLS handshake time (ms)',
      probe_http_connection_reused: 'Connection reused (1/0)',
      probe_http_flow_fanout: 'HTTP source-port fan-out',
  ...
  mform: {
    ...
    hostNewTitle: 'Add system status alert',
    hostEditTitle: 'Edit system status alert',
    hostSub: 'Watch the CPU, memory, load, network and disks of the machines this monitor group covers.',
    hostIntro: 'System status needs no IP or hostname: the metrics are already being reported by every agent granted the matching host.* permissions. All you set here is where the line is.',
    hostScopeHint: 'This alert applies to every agent in the monitor group chosen below — each machine is judged on its own readings.',
    hostDet: {
      title: 'What to watch',
      loading: 'Loading the stored thresholds…',
      loadErr: 'Could not read the stored thresholds ({err}). Reload before editing them — the values shown are defaults, not what is stored.',
      intro: 'Each machine is judged on its own. A reading has to stay at or above the level for the whole window before an alert is raised, and it clears once the machine settles back below it.',
      cpuTitle: 'CPU usage',
      cpuLead: 'Alert when CPU stays at or above',
      memTitle: 'Memory usage',
      memLead: 'Alert when memory stays at or above',
      loadTitle: 'System load',
      loadHint: 'Judged per logical core, so one setting reads correctly on a 4-core box and a 64-core one. Machines that do not report their core count are skipped.',
      loadLead: 'Alert when load stays at or above',
      loadPerCore: 'per core',
      diskTitle: 'Disk space',
      diskHint: 'Applies to every partition the agent reports; each one raises its own alert.',
      diskLead: 'Alert when any partition is at or above',
      netTitle: 'Network rate',
      netHint: 'Off by default: the right figure depends on your link speed, which nothing here can know. Leave a direction blank to not watch it.',
      netRx: 'Download above (Mbps)',
      netTx: 'Upload above (Mbps)',
      netBlank: 'not watched',
      forLead: 'for',
      minutes: '{n} min',
      minutesUnit: 'min',
      custom: 'Custom…',
      allOff: 'Nothing is being watched, so this monitor will never raise an alert.',
      savedOn: 'Watching {families}.',
      savedOff: 'Saved. Nothing is being watched yet.',
      saveErr: 'The monitor was saved, but its thresholds could not be written: {err}',
      errPct: 'Percentage thresholds must be between 1 and 100.',
      errLoad: 'The per-core load threshold must be greater than 0 and at most 100.',
      errDuration: 'Durations must be between 30 and 3600 seconds.',
      errNetDirection: 'Set a download or upload threshold, or switch network alerting off.',
      errNetRate: 'Network thresholds must be greater than 0 Mbps.',
    },
    secGeneral: 'General',
    secScope: 'Scope',
    ...
    sizeSweepTip:
      'When enabled, each cycle sends small, medium, and large packets (64, 512, and 1232 bytes) and compares their packet loss.\nIf large packets fail much more often than small ones, the cable, optic, or network adapter link may have a problem. This sends more ping packets per cycle.',
    ...
    httpFlowFanout: 'HTTP source-port fan-out',
    httpFlowFanoutTip:
      'When enabled, each cycle sends a complete HTTP request from several fixed local ports and checks the connection, TLS, status code, and keyword on every branch.\nA branch that repeatedly differs while the others pass shows inconsistent network or service paths. Higher levels make more requests; this supports direct GET/HEAD targets only and does not follow redirects.',
    httpFlowFanoutMethodOff: 'Fan-out is available only for GET or HEAD requests',
    httpFlowFanoutRedirectOff: 'Redirects are not followed during fan-out, so every branch checks the same target',
    tcpTls: 'Perform a TLS handshake after connect (SSL/TLS)',
    tcpFlowFanout: 'Source-port fan-out',
    flowFanoutOff: 'Off (regular probe)',
    flowFanoutLight: 'Light (4 branches)',
    flowFanoutStandard: 'Standard (8 branches)',
    flowFanoutDeep: 'Deep (16 branches)',
    flowFanoutMaximum: 'Maximum (32 branches)',
    tcpFlowFanoutTip:
      'When enabled, each cycle connects to the same target from several fixed local ports.\nIf the same connections keep failing while the others work, one path in a load balancer or link aggregation group may have a problem. Higher levels make more connections per cycle; this is unavailable through a proxy.',
  ...
  settings: {
    ...
    push: {
      ...
      hints: {
        api_base: 'Point this at a reverse proxy when api.telegram.org is unreachable directly.',
        uids: 'Separate multiple UIDs with commas or newlines, e.g. UID_xxx',
        topic_ids: 'Separate multiple topic IDs with commas (numbers)',
      },
      help: {
        dingtalk: 'Group settings → Group assistant → Add custom robot. With the "Signing" security mode, fill in the signing secret; with "Custom keywords", set the keyword to NetTact.',
        wecom: 'Group settings → Group robots → Add robot, then copy the key parameter out of the webhook URL.',
        feishu: 'Group settings → Group bots → Add custom bot, then paste the full webhook URL; fill in the secret when signature verification is on.',
        telegram: "Create a bot through {'@'}BotFather to get its token; the chat ID can be a numeric ID or an {'@'}channel name.",
        serverchan: 'Sign in to the ServerChan site to get your SendKey (both Turbo and ServerChan³ are supported).',
        wxpusher: 'Create an app in the WxPusher console to get its APP_TOKEN; UIDs are listed there once someone follows the app.',
      },
      helpLink: 'Official docs',
      secretEditHint: 'A saved secret shows as ••••••; leave it alone to keep it unchanged.',
      atLeastOneTarget: 'Fill in at least one of user UIDs and topic IDs.',
    ...
    channelManager: {
      add: 'Add channel',
      search: 'Search names or destinations',
      allTypes: 'All types',
      allStatuses: 'All statuses',
      enabled: 'Enabled',
      disabled: 'Disabled',
      summary: '{enabled} enabled, {total} channels total',
      colChannel: 'Channel',
      ...
      noMatch: 'No channels match the current filters.',
      stormMerged: 'Merge storms',
      individual: 'Send individually',
      unused: 'Not used by a policy',
      policyCount: '{count} policies',
      deleteConfirm: 'Delete notification channel "{name}"?',
      deleteConfirmUsed: '"{name}" is used by {count} notification policies. Those policies will stop sending through it after deletion. Delete it anyway?',
      drawerAddTitle: 'Add notification channel',
      drawerAddHint: 'Choose a destination, then configure its connection.',
      drawerEditTitle: 'Edit {name}',
      drawerEditHint: 'Change the connection, notification language and delivery behavior.',
    },
    channelCatalog: {
      title: 'Choose a channel type',
      hint: 'Each channel is one notification destination. Policies decide which faults are sent there.',
      back: 'Choose another type',
      enabledHint: 'Disabling keeps the configuration but stops deliveries.',
      emailRequired: 'SMTP host, sender and recipient are required.',
      groups: {
      ...
      descriptions: {
        webhook: 'Send to an HTTP endpoint with optional headers and body templates.',
        email: 'Deliver alerts to a mailbox through SMTP.',
        dingtalk: 'Send to a DingTalk custom group robot.',
        wecom: 'Send to a WeCom group robot.',
        feishu: 'Send to a Feishu custom group bot.',
        telegram: 'Send through a Telegram bot to a chat or channel.',
        serverchan: 'Push alerts to WeChat with a SendKey.',
        wxpusher: 'Push to WxPusher users or topics.',
        system: 'Show an operating-system notification on this NetTact computer.',
      },
    },
    ...
    listen: {
      ...
      envHint: 'The listen address is managed by the NETTACT_SERVER_ADDR environment variable and cannot be changed here.',
      envLockedHow: 'Change or remove NETTACT_SERVER_ADDR in the server environment, then restart the service. After removing it, you can configure the listen address here again.',
      flagHint: 'The listen address is managed by the -addr server startup flag and cannot currently be set here.',
      flagLockedHow: 'Change or remove -addr from the server startup command, then restart the service. After removing it, you can configure the listen address here again.',
    ...
    evidence: {
      ...
      hint: 'Each incident freezes one immutable base when it opens; the scenes beside it are collected locally by the Agents that detected the fault. These control how much of a scene the server stores and how long evidence is retained.',
      snapshotMax: 'Scene size cap',
      snapshotMaxHelp: 'Serialized size cap for one incident base and for each Agent scene (64–1024 KiB); over the cap, non-critical detail is truncated by priority.',
    ...
    diag: {
      ...
      hint: 'When a target stops answering, the Agent watching it runs one traceroute by itself and uploads the result with its telemetry — so the path is measured while the fault is happening, even if the Agent cannot reach this server at that moment. These settings are the policy it applies.',
      enable: 'Enable path diagnostics',
      enableHelp: 'When off, Agents run no automatic traceroute AND collect no incident scene for a probe fault — both hang off the same failure streak. Scenes collected when an Agent loses its connection to this server are unaffected.',
      ...
      consecutiveFailures: 'Failures before tracing',
      consecutiveFailuresHelp: 'Consecutive failed checks a target must accumulate before its Agent traces the path and collects an incident scene (1–20). Matches the fault confirmation threshold by default, so both run as the fault becomes real.',
      cooldown: 'Repeat interval',
      cooldownHelp: 'Minimum wait before the same destination is traced again on one Agent (1–1440 min). A target that stays down keeps failing every check, and the path to it does not change nearly that often.',
  ...
  agentStatus: {
    ...
    diskCapacityN: '{used} / {total} · {n} disks',
  ...
  metrics: {
    ...
    codeWindowLimited: 'Discrete diagnostics use raw observations, so this section shows the latest value from the last 2 days.',
    ...
    httpConnectionReused: 'Reused',
    httpConnectionNew: 'New connection',
    httpConnectionFoot: 'Last observed {time}',
    latestObservedFoot: 'Last observed {time}',
    ...
    metric: {
      ...
      probe_icmp_size_sweep: 'Size sweep',
      probe_dns_resolve_ms: 'Resolve',
      probe_dns_ok: 'Resolve OK',
      probe_http_status: 'Status',
      probe_http_latency_ms: 'Latency',
      probe_http_total_ms: 'Total time',
      probe_http_ttfb_ms: 'Time to first byte',
      probe_http_connect_ms: 'Connect time',
      probe_http_dns_ms: 'DNS resolve',
      probe_http_tls_ms: 'TLS handshake',
      probe_http_connection_reused: 'Connection reused',
      probe_http_ok: 'Request OK',
      probe_http_error_class: 'Request error',
      probe_http_flow_fanout: 'HTTP source-port fan-out',
      ...
      host_cpu_cores: 'Logical cores',
      // Derived kinds: no series carries these. They appear only as system-status
      // fault evidence, in the unit the threshold was authored in.
      host_load_per_core: 'Load per core',
      host_net_rx_mbps: 'Download rate (Mbps)',
      host_net_tx_mbps: 'Upload rate (Mbps)',
    },
    nat: {
      foot: 'Updated {time}',
    ...
    classifier: {
      ...
      probe_http_flow_fanout_0: 'Single request',
      probe_http_flow_fanout_1: 'Consistent',
      probe_http_flow_fanout_2: 'Stable branch mismatch',
      probe_http_flow_fanout_3: 'All branches failed',
      probe_http_flow_fanout_4: 'Insufficient evidence',
    },
    probeReason: {
      none: 'None',
  ...
  targetStatus: {
    ...
    agentTabMetrics: 'Host metrics',
    agentTabProcesses: 'Processes',
    agentTabConnections: 'Connections',
    collapseAgentList: 'Collapse Agent list',
    expandAgentList: 'Expand Agent list',
    ...
    connectionHistoryEmptyHint: 'This Agent has no online or offline transitions in the selected time range.',
    ...
    p95WindowHint: 'Available raw samples within the selected time range',
    natType: 'NAT type',
    ...
    availabilityEvidenceTitle: 'Availability and observation evidence',
    availabilityEvidenceHint: 'Availability uses server-judged probe rounds. Observation coverage shows where data actually falls inside the selected window; blank time is unknown.',
    selectedWindow: '{start} to {end}',
    thAgent: 'Agent',
    ...
    coverageSingle: '1 sample · {time}',
    coverageSpan: '{duration} covered · {n} samples',
    coverageAria: 'Observation coverage for {agent}: {coverage}',
    legendObserved: 'Observed normal',
    legendInterrupted: 'Observed interruption',
    legendUnknown: 'No data / unknown',
    ...
    fluctuationCount: '{n} fluctuation(s) in {window}',
    ...
    verdictRounds: '{rounds} successful rounds',
    noVerdictRounds: 'No verdict rounds',
  ...
  setup: {
    ...
    notifyWillReceive: 'Enabled · will receive alerts',
    notifyAddAction: 'Add another channel',
  ...
  chart: {
    ...
    restart: 'Restart',
    duration: 'Lasted {dur}',
    statusTimelineHint: 'Hover or tap a slice for its time; a dashed edge marks an issue',
    coverageNone: 'No observations in the selected window',
    coverageSingle: '1 sample · {time}',
    coverageSpan: '{duration} of data · {n} samples',
    coverageHint: 'Actual data span inside the selected window',
    loading: 'Loading chart…',
    noData: 'No data in this time window',
    noDataHint: 'The axis still represents the selected range. New observations will appear at their actual time.',
