...
// An in-tunnel report is PINNED to the tunnel long before it is known whether
// anything was measured over it: it can sit queued, be denied by permission, or
// refuse outright because the tunnel was re-keyed. The path note therefore has
// to describe the plan, and the "how to read these hops" guidance has to wait
// for hops — otherwise a refusal renders "this ran hop by hop inside the
// tunnel" directly above a reason saying no probe was ever sent.
...
const report = (over: Partial<TraceReportView> = {}): TraceReportView => ({
  ...
  deadline_at: '2026-08-01T12:01:00Z',
  subject_kind: 'target',
